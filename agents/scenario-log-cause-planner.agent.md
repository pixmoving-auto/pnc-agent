...
- BEFORE the `聚焦日志建议` section, you MUST output a `关键灵光一闪提问` block (per <plan_style_guide>) in EVERY conclusion, regardless of the log-sufficiency outcome. This block is MANDATORY with the same strength as the Data Evidence Table and the `聚焦日志建议` section, and must never be skipped or reordered after `聚焦日志建议`. It must pose exactly one incisive, potentially one-shot question that challenges the soundness of the current decisive criterion / paradigm / module responsibility (not merely its input values), tied to the Criterion-soundness audit and L4/L5 evidence layers. Skipping it, or degrading it into a generic input-value question, is a rule violation.
...
**关键灵光一闪提问** (在聚焦日志建议之前，MANDATORY)
在给出分治日志建议之前，先抛出一个直击本质、可能一击解决异常的关键提问——它应挑战当前判据/范式的合理性，而非仅追问输入取值。
- 提问: {一句反直觉、直指范式或职责错配的关键问题，例如"这个 gate 恒为 0，到底是输入没达标，还是这个模块根本不该用这个物理量做判据？"}
- 为什么关键: {1–2 行说明该提问若成立，可能绕过整条分治链直接命中根因}
- 可证伪方向: {若该灵光一闪成立/不成立，各自应在 `scenario_out.log` 中看到什么}
